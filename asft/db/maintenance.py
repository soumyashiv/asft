...
def prune_routing_history():
    ...
    with SessionLocal() as db:
        ...
        record_10k = (
            db.query(RoutingHistory.timestamp)
            .order_by(RoutingHistory.timestamp.desc())
            .offset(9999)
            .first()
        )
        ...
        old_records = (
            db.query(RoutingHistory).filter(RoutingHistory.timestamp < delete_threshold).all()
        )
        ...
        for r in old_records:
            ...
            if key not in aggregates:
                aggregates[key] = {"acc": [], "cost": [], "runtime": [], "successes": 0, "total": 0}
        ...
        for (task_hash, strategy), stats in aggregates.items():
            outcome = (
                ...
                .filter(
                    StrategyOutcome.task_hash == task_hash, StrategyOutcome.strategy == strategy
                )
                .first()
            )
