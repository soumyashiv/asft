...
class StreamingCompressor:
    ...
    def __init__(self, config=None):
        ...
        self.clusterer = MiniBatchKMeans(
            n_clusters=self.n_clusters, batch_size=self.batch_size, random_state=42
        )

        # FAISS Deduplication index
        self.similarity_threshold = (
            getattr(config, "similarity_threshold", 0.05) if config else 0.05
        )
    ...
    def check_memory(self):
        ...
        if mem_mb >= self.limit_abort:
            logger.critical(
                f"MEMORY ABORT: {mem_mb:.1f} MB >= {self.limit_abort} MB. Aborting to save OS."
            )
            ...
            logger.error(
                f"MEMORY EMERGENCY: {mem_mb:.1f} MB >= {self.limit_emergency} MB. Reducing batch size."
            )
            ...
            logger.warning(
                f"MEMORY FLUSH: {mem_mb:.1f} MB >= {self.limit_flush} MB. Flushing to disk."
            )
            self._flush_state_to_disk()
            import gc
    ...
    def _flush_state_to_disk(self):
        ...
        with open(temp_path, "a", encoding="utf-8") as f:
            for cluster_id, items in self.reservoir.items():  # noqa: B007
                for item in items:
    ...
    def compress_stream(
        ...
        try:
            ...
            if data_files:
                dataset = load_dataset(
                    dataset_format, data_files=data_files, split="train", streaming=True
                )
            else:
        ...
        for item in dataset:
            ...
            if len(batch_texts) >= self.batch_size:
                ...
                self.check_memory()
                logger.info(
                    f"Processed {total_processed} items. Memory: {self.process.memory_info().rss / (1024*1024):.1f} MB"
                )
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            for _cluster_id, items in self.reservoir.items():
                for item in items:
                    f.write(json.dumps(item, ensure_ascii=False) + "\n")
                    final_count += 1
        ...
        logger.info(
            f"Streaming compression complete. Reduced {total_processed} -> {final_count}. Mem: {report['peak_memory_mb']:.1f} MB"
        )
    ...
    def _process_batch(self, texts: list[str], metas: list[dict[str, Any]], model):
        ...
        for i, (text, meta, label) in enumerate(  # noqa: B007
            zip(novel_texts, novel_metas, labels, strict=False)
        ):
            item = {"text": text}
            if meta:
                item["metadata"] = meta
