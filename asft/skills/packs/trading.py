...
class TradingSkillPack(SkillPack):
    def __init__(self, pack_dir=None):
        ...
        self.meta.description = "Financial market analysis, trading signals, risk assessment, and portfolio management"
    ...
    def evaluate(self, sample_input: str, sample_output: str) -> float:
        has_analysis = any(w in sample_output.lower() for w in
                           ["support", "resistance", "trend", "volume", "momentum", "risk"])
        has_disclaimer = "not financial advice" in sample_output.lower() or "analysis only" in sample_output.lower()
        has_numbers = any(c.isdigit() for c in sample_output)
        return min(1.0, (0.4 if has_analysis else 0.1) + (0.3 if has_numbers else 0) + (0.3 if has_disclaimer else 0))
    ...
    def _estimate_confidence(self, output: str) -> float:
        if not output or len(output) < 30: return 0.1
        score = 0.4
        if len(output) > 200: score += 0.2
        if any(w in output.lower() for w in ["risk", "trend", "signal"]): score += 0.2
        if any(c.isdigit() for c in output): score += 0.2
