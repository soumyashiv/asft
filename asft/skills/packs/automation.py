...
class AutomationSkillPack(SkillPack):
    def __init__(self, pack_dir=None):
        ...
        self.meta.description = (
            "Workflow automation, scripting, pipeline design, and system orchestration"
        )
    ...
    def evaluate(self, sample_input: str, sample_output: str) -> float:
        ...
        has_error_handling = any(
            w in sample_output.lower() for w in ["error", "except", "try", "retry", "fallback"]
        )
        return min(
            1.0,
            (0.4 if has_code else 0.1)
            + (0.2 if has_steps else 0)
    ...
    def _estimate_confidence(self, output: str) -> float:
        ...
        if "```" in output:
            score += 0.3
        if any(w in output.lower() for w in ["step", "workflow", "pipeline"]):
            score += 0.2
        if len(output) > 300:
            score += 0.2
    ...
    def _detect_type(self, text: str) -> str:
        ...
        if any(w in text_lower for w in ["docker", "kubernetes", "deploy"]):
            return "devops"
        if any(w in text_lower for w in ["cron", "schedule", "trigger"]):
            return "scheduling"
        if any(w in text_lower for w in ["api", "webhook", "http"]):
            return "integration"
        return "general"
