...
class CostEstimator:
    ...
    """
    def estimate(self, strategy: str, dataset_size: int, model_size_params: int, task_hash: str) -> float:
        with SessionLocal() as db:
            results = db.query(RoutingHistory).filter(
...
class MultiArmedBanditRouter:
    ...
    """
    def __init__(self, epsilon_initial: float = 0.50, epsilon_min: float = 0.05, alpha: float = 0.995):
    ...
    def _get_historical_utilities(self, db: Session, task_hash: str) -> tuple[dict[str, float], int]:
        ...
        results = db.query(RoutingHistory).filter(RoutingHistory.task_hash == task_hash, RoutingHistory.success == True).all()
...
class OptimizerDecisionEngine:
    ...
    def evaluate_workload(self, task_type: str, dataset_size: int, model_size: int) -> dict[str, Any]:
        ...
        if not adapter.is_healthy() and "memory_rag" in available_strategies:
            logger.warning("SecureQdrantAdapter health check failed. Removing memory_rag from optimizer strategies.")
    ...
    def _record_decision(self, task_hash: str, strategy: str, is_explore: bool, expected_cost: float) -> str:
